<script lang="ts">
	...
	const LAMBDA_URL = 'https://oo3l4sdaz6gbpfkd3cprdz4ne40piukc.lambda-url.eu-central-1.on.aws';
...
<section class="container mx-auto px-4 sm:px-6 lg:px-8 py-1">
				...
				<div class="flex items-center justify-between gap-3">
					<div>
						...
						<p class="text-gray-600 text-sm">
							Ask me anything about Adam's experience, skills, or tech topics
						</p>
					</div>
					<button
						type="button"
						on:click={clearChat}
						disabled={isLoading}
						class="inline-flex items-center gap-2 px-3 sm:px-4 min-h-[44px] bg-white border border-gray-200/70 text-gray-700 rounded-xl shadow-sm hover:bg-gray-50 hover:shadow-md active:shadow-sm transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
					>
						<svg
			...
			>
				{#each messages as message (message)}
					...
					>
						...
						{/if}
						<div
							class="max-w-[85%] sm:max-w-[75%] lg:max-w-[70%] rounded-2xl px-4 py-3 shadow-sm {message.role ===
							'user'
						...
						>
							<div class="whitespace-pre-wrap break-words leading-relaxed text-sm sm:text-base">
								{message.content ||
									(isLoading && message.role === 'assistant' ? 'Thinking...' : '')}
							</div>
   ...
   <div class="flex-shrink-0 pt-4 mt-4 border-t border-gray-200/50">
                <form on:submit|preventDefault={askAssistant} class="flex gap-3 items-stretch">
                    <div class="flex-1 relative">
                        <textarea
                            ...
                            disabled={isLoading}
                            rows="1"
                            class="w-full px-4 py-3 bg-white border border-gray-200/50 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none text-gray-900 placeholder:text-gray-400 disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
